...
class RpnParserTest {
    ...
    void testParseOperation()
        ...
        Expression expr = RpnParser.parse("2 10 1 - *", Map.of());
        assertInstanceOf(Operation.class, expr);
        assertEquals(18.0, expr.eval(MapVarTable.empty()));
    ...
    void testParseIncomplete() {
        assertThrows(IncompleteRpnException.class, () -> RpnParser.parse("4 3 6 -", Map.of()));
    }
    ...
    void testParseUnderflow() {
        assertThrows(IncompleteRpnException.class, () -> RpnParser.parse("2 2 + -", Map.of()));
    }
